 ...
 */
export function collides(l1: LayoutItem, l2: LayoutItem, allowOverlap: ?boolean): boolean {
  if (allowOverlap) return false // no collisions if overlap is allowed;
 ...
 */
export function getFirstCollision(layout: Layout, layoutItem: LayoutItem, allowOverlap: ?boolean): ?LayoutItem {
  if (allowOverlap) return undefined
  for (let i = 0, len = layout.length; i < len; i++) {
    if (collides(layout[i], layoutItem), allowOverlap) return layout[i];
  }
}

export function getAllCollisions(layout: Layout, layoutItem: LayoutItem, allowOverlap: ?boolean): Array<LayoutItem> {
  return allowOverlap ? [] : layout.filter((l) => collides(l, layoutItem, allowOverlap));
}
 ...
 * @param  {Boolean}    [allowOverlap] If true, indicates that the grid allows elements to overlap.
 */
export function moveElement(layout: Layout, l: LayoutItem, x: ?number, y: ?number, isUserAction: ?boolean, allowOverlap: ?boolean): Layout {
  ...
  const collisions = getAllCollisions(sorted, l, allowOverlap);
  ...
  for (let i = 0, len = collisions.length; i < len; i++) {
    ...
    if (collision.static) {
      layout = moveElementAwayFromCollision(layout, collision, l, isUserAction, allowOverlap);
    } else {
      layout = moveElementAwayFromCollision(layout, l, collision, isUserAction, allowOverlap);
    }
  }
...
export function moveElementAwayFromCollision(layout: Layout, collidesWith: LayoutItem,
                                             itemToMove: LayoutItem, isUserAction: ?boolean, allowOverlap: ?boolean): Layout {
